...
(function ($) {
	...
	// Mobile menu - only initialize if header exists, otherwise let header-loader handle it
	if (document.querySelector(".header-area nav")) {
		$(".header-area nav").meanmenu();
	}
	// Menu Fixed
	const fixed_top = document.querySelector('.header-area');
	window.addEventListener('scroll', function() {
		if (window.scrollY > 300) {
			fixed_top.classList.add('menu-fixed', 'animated', 'fadeInDown');
		} else {
			fixed_top.classList.remove('menu-fixed', 'fadeInDown');
		}
	});
	...
	// Temporarily disabled - testing direct embeds
	...
	// 		var modalHtml = '<div id="videoModal" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 9999; display: flex; align-items: center; justify-content: center;">' +
	// 			'<div style="position: relative; width: 80%; max-width: 800px; height: 450px;">' +
	// 			'<button id="closeModal" style="position: absolute; top: -40px; right: 0; background: none; border: none; color: white; font-size: 30px; cursor: pointer;">&times;</button>' +
	// 			'<iframe width="100%" height="100%" src="' + embedUrl + '" frameborder="0" allowfullscreen allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin"></iframe>' +
	// 			'</div></div>';
	//
